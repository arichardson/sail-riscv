...
unit mem_tag_write_callback(uint64_t paddr, bool value)
{
  if (config_print_mem_access) {
    fprintf(trace_log, "tag[0x%016" PRIX64 "] <- %d", paddr, (int)value);
  }
  return UNIT;
}
...
unit mem_tag_read_callback(uint64_t paddr, bool value)
{
  if (config_print_mem_access) {
    fprintf(trace_log, "tag[R,0x%016" PRIX64 "] -> %d", paddr, (int)value);
  }
  return UNIT;
}
